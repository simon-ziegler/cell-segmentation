...
def compute_gaussian_list(img_intens_list,mask_list,num_gaussians=2,invert_mask=False,equalize=False,high_quantile=1.0):
    """ computes gaussian distributions for pixel intensities in cells

        Parameters
        ----------

        img_intens_list : list of OpenCV images
            images with cell intensities

        mask_list : list of OpenCV images
            images with mask of cells

        num_gaussians : integer
            number of gaussians computed for intensities

        invert_mask : boolean
            if True, mask is inverted

        equalize : Boolean
            if True, image is equalized

        high_quantile : float
            quantile for image intensity, pixels with higher intensities are sorted out
        ...
        means : list of floats
            gaussian means of intensities
            
        sigma : list of floats
            gaussian variances of intensities

        X_flat : numpy array
            flat array of pixel intensities
...
def classify_gaussians(img_intens,mask,means,sigma,img_show_rgb,mask_id_list=[]):
    """ classification features of cells by intensities with intensity means and gray level co-occurance matrices

        Parameters
        ----------

        img_intens: OpenCV image
            cell image
        ...
        means : list of floats
            means of cell pixel intensities

        sigma : list of floats
            variances of cell pixel intensities

        img_show_rgb : OpenCV image
            classification result for gaussians    

        mask_id_list : list of integers
            list with mask ids
        ...
        mask_class_dict: dictionary
            classification result of cells

        cell_means : list of floats
            pixel intensity means of cells

        glcm_homo : list of floats
            gray level co-occurance matrix homogenitey of cells

        glcm_entropy : list of floats
            gray level co-occurance matrix entropy of cells

        glcm_energy : list of floats
            gray level co-occurance matrix energy of cells
...
def draw_classes(img,contours,mask_class,thickness=2):
    """draw classification in image
        ...
        mask_class : list of integers
            cell class

        thickness : integer
            thickness of drawed outlines


        Returns
        -------

        img_cls : OpenCV image
            image with classification results in outlines
...
def load_images_result(res_dir):
    """load results of cell segmentation
    
        Parameters
        ----------

        res_dir : string
            result directory of cell segmentation


        Returns
        -------

        img_orig : list of OpenCV images
            basis images for cell identification

        img_orig_intens : list of OpenCV images
            red channel images

        img_orig_aav : list of OpenCV images
            green channel images

        mask_list : list of OpenCV images
            masks for cells in basis images

        mask_red_list : list of OpenCV images
            masks for cells in red channel images

        mask_green_list : list of OpenCV images
            masks for cells in green channel images

        f_names_load_img : list of strings
            file names of loaded basis image files
...
def plot_gaussians(means,sigma,rawdata,plt,legends=None):
    """"plot guassians of pixel intensities

        Parameters
        ----------

        means : list of float
            list of gaussian means for pixel intensities            

        sigma : list of float
            list of gaussian variances for pixel intensities

        rawdata : numpy array
            raw pixel data

        plt : matplotlib plot
            matplotlib plot of gaussians

        legends : matplotlib legends
            matplotlib legends of plot 
    """
...
def write_mask_id(img,mask,min_cell_size=10):
    """write mask information in image and get cell features
        ...
        mask : OpenCV image
            image with mask ID information

        min_cell_size : integer
            minimal pixel number of cells


        Returns
        -------
    
        img_id_rgb : OpenCV image
            image with mask contours and ID

        centers : list of tuples
            center position of cells

        perimeters : list of float
            perimeters of cells

        areas : list of floats
            area in pixels of cells
...
def get_output_table(centers,perimeters,areas,red_cell_means,green_cell_means,mask_class_red_one,mask_class_green_one):    
    """create table with cell information
        ...
        red_cell_means : list of floats
            means of intensities for cells in red channel images

        green_cell_means : list of floats
            means of intensities for cells in green channel images

        mask_class_red_one : list of integers
            classes of cells in red channel images

        mask_class_green_one : list of integers
            classes of cells in green channel images

        Returns
        -------

        output table : list of strings
...
def sortout_homo(mask_class_list,cell_homo,thres=0.1):
    """sort out cells by homogeneity

        Parameters
        ----------

        mask_class_list : dictionary of integers
            dictionary of mask class

        cell_homo : dictionary of integers
            dictionary of mask homogeneity
        ...
        mask_list_sortout : dictionary of integers
            dictionary of mask class with homogeneity classification
    """
...
def classifiy_means_homo(cell_means,cell_homo,changed_by_hand,means,sigma,thres=0.1):
    """classify cells by mean intensity and homogeneity

        Parameters
        ----------

        cell_means : dictionary of floats
            mean intensity for cells

        cell_homo : dictionary of floats
            homogeneity of cells

        changed_by_hand : dictionary of integers
            manually modified classes

        means : list of floats
            gaussian means for image intensities

        sigma : list of floats
            gaussian variances for image intensities
            if variance is lower zero, mean is interpreted as threshold
        ...
        mask_class_list : dictionary of integers
            dictionary of classes for cells
